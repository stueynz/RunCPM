...
//    returns next free buffer location
//    EXTENDED_DEBUG needs hex dump in buffer rather than direct to console
char *
...
/* Disassemble instruction at given address into given buffer.
   Returns the number of bytes disassembled.  
   EXTENDED_DEBUG needs disassembly in buffer rather than direct to console */
uint8 bDisasm(uint16 pos, char *bptr) {
    ...
    for (uint8 i = 0; i < len; ++i) {
        bptr = _bputhex8(_RamRead((pos + i) & 0xffff), bptr);
        *bptr++ = ' ';
    }
...
/**  Extended debugger has complex commandline, and a GDB Server */
