...
static void disassemble_main( uint16_t address, char *buffer,
			      size_t buflen, size_t *length,
			      enum hl_type use_hl );
static void disassemble_00xxxxxx( uint16_t address, char *buffer,
				  size_t buflen, size_t *length,
				  enum hl_type use_hl );
static void disassemble_00xxx010( uint16_t address, char *buffer,
				  size_t buflen, size_t *length,
				  enum hl_type use_hl );
static void disassemble_00xxx110( uint16_t address, char *buffer,
				  size_t buflen, size_t *length,
				  enum hl_type use_hl );
static void disassemble_11xxxxxx( uint16_t address, char *buffer,
				  size_t buflen, size_t *length,
				  enum hl_type use_hl );
static void disassemble_11xxx001( uint8_t b, char *buffer,
				  size_t buflen, size_t *length,
				  enum hl_type use_hl );
static void disassemble_11xxx011( uint16_t address, char *buffer,
				  size_t buflen, size_t *length,
				  enum hl_type use_hl );
static void disassemble_11xxx101( uint16_t address, char *buffer,
				  size_t buflen, size_t *length,
				  enum hl_type use_hl );
static void disassemble_cb( uint16_t address, char *buffer,
			    size_t buflen, size_t *length );
static void disassemble_ed( uint16_t address, char *buffer,
			    size_t buflen, size_t *length );
static void disassemble_ddfd_cb( uint16_t address, char offset,
				 enum hl_type use_hl, char *buffer,
				 size_t buflen, size_t *length );

static void get_byte( char *buffer, size_t buflen, uint8_t b );
static void get_word( char *buffer, size_t buflen, uint16_t address );
static void get_offset( char *buffer, size_t buflen, uint16_t address,
			uint8_t offset );

static const char *reg_pair( uint8_t b, enum hl_type use_hl );
static const char *hl_ix_iy( enum hl_type use_hl );
static void ix_iy_offset( char *buffer, size_t buflen, enum hl_type use_hl,
			  uint8_t offset );

static int source_reg( uint16_t address, enum hl_type use_hl,
		       char *buffer, size_t buflen );
static int dest_reg( uint16_t address, enum hl_type use_hl,
		     char *buffer, size_t buflen );
static int single_reg( int i, enum hl_type use_hl, uint8_t offset,
		       char *buffer, size_t buflen );

static const char *addition_op( uint8_t b );
static const char *condition( uint8_t b );
static const char *rotate_op( uint8_t b );
static const char *bit_op( uint8_t b );
static int bit_op_bit( uint8_t b );
...
static void
disassemble_main( uint16_t address, char *buffer, size_t buflen,
...
static void
disassemble_00xxxxxx( uint16_t address, char *buffer, size_t buflen,
		      size_t *length, enum hl_type use_hl )
{
  ...
  case 0x00: case 0x08:
    if( b <= 0x08 ) {
      ...
      get_offset( buffer2, 40, address + 2, _RamRead( address + 1 ) );
  ...
  case 0x06: case 0x0e:
    *length = 2 + dest_reg( address, use_hl, buffer2, 40 );
    get_byte( buffer3, 40, _RamRead( address + *length - 1 ) );
    snprintf( buffer, buflen, "LD %s,%s", buffer2, buffer3 );
    break;
...
static void
disassemble_00xxx010( uint16_t address, char *buffer, size_t buflen,
		      size_t *length, enum hl_type use_hl )
{
  char buffer2[40];
  uint8_t b = _RamRead( address );
...
static void
disassemble_00xxx110( uint16_t address, char *buffer, size_t buflen,
		      size_t *length, enum hl_type use_hl )
{
  char buffer2[40];
  uint8_t b = _RamRead( address );
...
static void
disassemble_11xxxxxx( uint16_t address, char *buffer, size_t buflen,
		      size_t *length, enum hl_type use_hl )
{
  char buffer2[40];
  uint8_t b = _RamRead( address );
  ...
  case 0x06:
    get_byte( buffer2, 40, _RamRead( address + 1 ) );
    snprintf( buffer, buflen, addition_op( b ), buffer2 );
    *length = 2;
    break;
...
static void
disassemble_11xxx001( uint8_t b, char *buffer, size_t buflen,
		      size_t *length, enum hl_type use_hl )
{
  switch( ( b >> 3 ) - 0x18 ) {
...
static void
disassemble_11xxx011( uint16_t address, char *buffer, size_t buflen,
		      size_t *length, enum hl_type use_hl )
{
  char buffer2[40];
  uint8_t b = _RamRead( address );
  ...
  case 0x01:
    if( use_hl != USE_HL ) {
      char offset = _RamRead( address + 1 );
...
static void
disassemble_11xxx101( uint16_t address, char *buffer, size_t buflen,
		      size_t *length, enum hl_type use_hl )
{
  char buffer2[40];
  uint8_t b = _RamRead( address );
...
static void
disassemble_cb( uint16_t address, char *buffer, size_t buflen,
		size_t *length )
{
  char buffer2[40];
  uint8_t b = _RamRead( address );
...
static void
disassemble_ed( uint16_t address, char *buffer, size_t buflen,
		size_t *length )
{
  uint8_t b;
  char buffer2[40];
...
static void
disassemble_ddfd_cb( uint16_t address, char offset,
...
static void
get_byte( char *buffer, size_t buflen, uint8_t b )
{
...
static void
get_word( char *buffer, size_t buflen, uint16_t address )
{
  uint16_t w;

  w  = _RamRead( address + 1 ); w <<= 8;
  w += _RamRead( address     );
...
static void
get_offset( char *buffer, size_t buflen, uint16_t address,
	    uint8_t offset )
{
...
static const char *
reg_pair( uint8_t b, enum hl_type use_hl )
{
  switch( ( b >> 4 ) & 0x03 ) {
  case 0: return "BC";
  case 1: return "DE";
...
static int
source_reg( uint16_t address, enum hl_type use_hl, char *buffer,
	    size_t buflen )
{
  return single_reg( _RamRead( address ) & 0x07, use_hl,
		     _RamRead( address + 1 ), buffer, buflen );
}

/* Get an 8-bit register, based on bits 3-5 of the opcode at 'address' */
static int
dest_reg( uint16_t address, enum hl_type use_hl, char *buffer,
	  size_t buflen )
{
  return single_reg( ( _RamRead( address ) >> 3 ) & 0x07, use_hl,
		     _RamRead( address + 1 ), buffer, buflen );
}
...
static int
single_reg( int i, enum hl_type use_hl, uint8_t offset,
	    char *buffer, size_t buflen )
{
  char buffer2[40];
...
uint16_t
debugger_search_instruction( uint16_t address, int delta )
{
...
uint8_t test1_data[] = { 0x00 };

uint8_t test2_data[] = { 0xdd, 0x00 };
uint8_t test3_data[] = { 0xdd, 0x09 };
uint8_t test4_data[] = { 0xdd, 0xdd, 0x00 };
uint8_t test5_data[] = { 0xdd, 0xcb, 0x55, 0x06 };

uint8_t test6_data[] = { 0xfd, 0x00 };
uint8_t test7_data[] = { 0xfd, 0x09 };
uint8_t test8_data[] = { 0xfd, 0xfd, 0x00 };
uint8_t test9_data[] = { 0xfd, 0xcb, 0x55, 0x06 };

uint8_t test10_data[] = { 0xdd, 0xfd, 0x09 };
uint8_t test11_data[] = { 0xfd, 0xdd, 0x09 };

uint8_t test12_data[] = { 0xdd, 0xfd, 0xdd, 0xfd, 0xdd, 0xfd, 0xdd,
                                   0xfd, 0xdd, 0xfd, 0xdd, 0xfd, 0x09 };
uint8_t test13_data[] = { 0xfd, 0xdd, 0xfd, 0xdd, 0xfd, 0xdd, 0xfd,
                                   0xdd, 0xfd, 0xdd, 0xfd, 0xdd, 0x09 };

uint8_t test14_data[] = { 0x7e };
uint8_t test15_data[] = { 0xdd, 0x7e, 0x55 };

static int
run_test( uint8_t *data, size_t data_length, const char *expected )
{
  char disassembly[16];
  size_t length;
