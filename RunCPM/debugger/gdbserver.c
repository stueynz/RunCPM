...
static volatile char num_attached_clients = 0;
static volatile char break_on_no_clients = 0;
...
struct action_set_registers_args_t {
    uint16_t regs_data[sizeof(registers) / sizeof(uint16_t*)];
};
...
static int set_register_value(int reg, uint16_t value)
{
    if (reg >= (sizeof(registers) / (sizeof(uint16_t*))))
    {
        return 1;
    }
    *registers[reg] = value;
...
static int get_register_value(int reg, uint16_t* result)
{
    if (reg >= (sizeof(registers) / (sizeof(uint16_t*))))
    {
        return 1;
...
static uint8_t process_packet()
{
    ...
    switch (request)
    {
        ...
        case 'G':
        {
            ...
            if (strlen(payload) != ((sizeof(registers) / sizeof(uint16_t*)) * 4))
            {
                write_packet("E01");
                break;
            }
            hex2mem(payload, (void *)&r.regs_data, (sizeof(registers) / sizeof(uint16_t*)) * 2);
...
{
    // utils_networking_init();  -- Not required when running under POSIX
...
static uint8_t action_get_registers(const void* arg, void* response)
{
    ...
    uint16_t regs[sizeof(registers) / (sizeof(uint16_t*))];
  
    for (i = 0; i < sizeof(registers) / (sizeof(uint16_t*)); i++)
    {
        get_register_value(i, &regs[i]);
    }
  
    mem2hex((const uint8_t *)regs, resp_buff, (sizeof(registers) / sizeof(uint16_t*)) * 2);
...
static uint8_t action_get_mem(const void* arg, void* response)
{
    ...
    uint16_t address = mem->maddr;
    for (i = 0; i < mem->mlen; i++, address++)
    {
        uint8_t data = _RamRead(address);
