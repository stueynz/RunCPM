...
static uint32_t evaluate_unaryop( struct unaryop_type *unaryop );
static uint32_t evaluate_binaryop( struct binaryop_type *binary );
...
debugger_expression_new_number( uint32_t number)
{
  ...
  exp = (debugger_expression *) malloc( sizeof(debugger_expression));
...
debugger_expression_new_variable( const char *name)
{
  ...
  exp->precedence = PRECEDENCE_ATOMIC;
  exp->types.variable = strdup(name);
...
debugger_expression_copy( debugger_expression *src )
{
  ...
  dest = (debugger_expression *) malloc( sizeof(debugger_expression));
  if( !dest ) return NULL;
...
evaluate_unaryop( struct unaryop_type *unary )
{
  ...
  case DEBUGGER_TOKEN_DEREFERENCE:
    debugger_expression_deparse(buf, 80, unary->op);
    int addr = debugger_expression_evaluate(unary->op);
    fprintf(stderr, "op:%s   addr:%04x\n", buf, addr);
    return _RamRead( debugger_expression_evaluate( unary->op ) );
...
evaluate_binaryop( struct binaryop_type *binary )
{
  ...
  case '/': {
      uint32_t op2 = debugger_expression_evaluate( binary->op2 );
      if( op2 == 0 ) {
		 ...
		 const struct unaryop_type *unaryop )
{
  ...
  operand_buffer = (char *) malloc( length );
		  ...
		  const struct binaryop_type *binaryop )
{
  ...
  operand1_buffer = (char *) malloc( 2 * length);
...
is_non_associative( int operation )
{
  ...
  default:
    ui_error( UI_ERROR_ERROR, "unknown associative operation %d", operation );
    Status=1;
    return PRECEDENCE_ATOMIC;    
  }
