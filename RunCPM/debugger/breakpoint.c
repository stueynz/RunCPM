...
/* Add a breakpoint */
int
debugger_breakpoint_add_address( debugger_breakpoint_type type, 
                                 // int source,
                                 // int page, 
                                 uint16_t offset,
		...
		debugger_expression *condition )
{
  ...
  bp = (debugger_breakpoint *) malloc(sizeof(debugger_breakpoint));
		  ...
		  uint32_t value )
{
  ...
  switch( type ) {
    ...
    /* Address must match for memory operations */
  case DEBUGGER_BREAKPOINT_TYPE_EXECUTE:
  case DEBUGGER_BREAKPOINT_TYPE_READ:
  case DEBUGGER_BREAKPOINT_TYPE_WRITE:
...
int
debugger_breakpoint_clear( uint16_t address )
{
  GSList *ptr;
  gpointer ptr_data;
...
{
  const debugger_breakpoint *bp = data;
  uint16_t address = *(const uint16_t*)user_data;
...
debugger_breakpoint_set_commands( size_t id, const char *commands )
{
  ...
  free( bp->commands );
  bp->commands = strdup(commands);          //utils_safe_strdup( commands );
...
void
debugger_breakpoint_time_fn( uint32_t tstates, int type GCC_UNUSED,
