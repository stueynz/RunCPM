...
command:   BASE number { debugger_output_base = $2; }
	 | breakpointlife breakpointtype breakpointlocation optionalcondition {
             debugger_breakpoint_add_address( $2, 
			 								  /* $3.source, $3.page, */
											  $3.offset,
                                              0, $1, $4 );
	   }
...
*/	   
	 ...
	 | CONDITION NUMBER expressionornull { debugger_breakpoint_set_condition( $2, $3 ); }
	 ...
	 | DEBUGGER_PRINT number { fprintf(stderr, "\n0x%x/0%o/%d\n", $2, $2, $2 ); }
...
breakpointlocation:   numberorpc { $$.offset = $1; }


/* We're not doing STRING values (which are for SOURCE & PAGE based addressing)
...
expression:   NUMBER { $$ = debugger_expression_new_number( $1 );
            ...
            | STRING ':' STRING { $$ = debugger_expression_new_system_variable( $1, $3);
                                  if( !$$ ) YYABORT;
                                }
	    | VARIABLE { $$ = debugger_expression_new_variable( $1 );
			 if( !$$ ) YYABORT;
		       }
	    | '(' expression ')' { $$ = $2; }
            | '[' expression ']' {
                $$ = debugger_expression_new_unaryop( DEBUGGER_TOKEN_DEREFERENCE, $2);
                if( !$$ ) YYABORT;
              }
	    ...
	    | expression LOGICAL_AND expression {
	        $$ = debugger_expression_new_binaryop(DEBUGGER_TOKEN_LOGICAL_AND, $1, $3);
		if( !$$ ) YYABORT;
	      }
